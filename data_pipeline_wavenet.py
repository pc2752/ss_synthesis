    ...
    if mode == "Train":
        ...
        for k in range(config.batches_per_epoch_train):
            ...
            for i in range(max_files_to_process):
                ...
                if augment:
                    ...
                    # feats = np.pad(feats, ((config.rec_field,0),(0,0)),'constant',constant_values=(0, 0))
                    ...
                    for j in range(config.samples_per_file):
                            voc_idx = np.random.randint(0,len(voc_stft)-config.max_phr_len-1)
                            bac_idx = np.random.randint(0,len(back_stft)-config.max_phr_len-1)
                            mix_stft = voc_stft[voc_idx:voc_idx+config.max_phr_len+1,:] + back_stft[bac_idx:bac_idx+config.max_phr_len+1,:]*np.clip(np.random.rand(1),0.0,0.9)
                            targets.append(feats[voc_idx:voc_idx+config.max_phr_len+1,:])
                    ...
                    for j in range(config.samples_per_file):
                        ...
                        inputs.append(mix_stft[voc_idx:voc_idx+config.max_phr_len+1,:])
        ...
        for k in range(config.batches_per_epoch_val):
            ...
            for i in range(max_files_to_process):
                    ...
                    for j in range(config.samples_per_file):
                        voc_idx = np.random.randint(0,len(mix_stft)-config.max_phr_len-1)
